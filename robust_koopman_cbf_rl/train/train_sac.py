"""Unconstrained SAC baseline (no safety filter)."""
...
def main(env_cfg_path, sac_cfg_path, log_dir, total_steps=None, checkpoint_every=50_000,
         seed=None):
    ...
    logger = CSVLogger(Path(log_dir) / "sac_baseline.csv")
    ...
    try:
        ...
        for step in range(sac_cfg.total_steps):
            ...
            if checkpoint_every > 0 and step > 0 and step % checkpoint_every == 0:
                agent.save(Path(log_dir) / f"sac_baseline_step{step}.pt")
    finally:
        agent.save(Path(log_dir) / "sac_baseline_final.pt")
        try:
...
if __name__ == "__main__":
    ...
    a = ap.parse_args()
    main(a.env_cfg, a.sac_cfg, a.log_dir, a.total_steps, a.checkpoint_every, seed=a.seed)
